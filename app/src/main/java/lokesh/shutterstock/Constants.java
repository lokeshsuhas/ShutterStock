package lokesh.shutterstock;

/**
 * Created by Lokesh on 03-03-2016.
 */
public class Constants {
    public final static String API_BASE_URL = "https://api.shutterstock.com/v2/";
    public final static String CLIENT_ID = "564b2cb0f3ee2528c701";
    public final static String CLIENT_SECRET = "0e39b81135fba4554e69fa336a73fc6112896b02";
    public final static String AC_GRANT_TYPE = "authorization_code";
    public final static String CC_GRANT_TYPE = "client_credentials";
    public final static String REDIRECT_URL = "http://localhost";
    public final static String SP_ACCESS_TOKEN = "accesstoken";
    public final static Integer PER_PAGE=10;
    public final static Integer COLUMN_PORTRAIT=3;
    public final static Integer COLUMN_LANDSCAPE=4;
}
